...
def test_truncated_one_dimension_logistic_regression_no_intercept():
    ...
    ch.manual_seed(seed)
    trunc_log_reg = TruncatedLogisticRegression(args, phi, alpha, fit_intercept=False)
    ...
    delphi_log_reg_cos_sim = float(
        cos_sim(delphi_log_reg_[None, ...], sklearn_[None, ...])
    )
    print(f"delphi cos sim: {delphi_log_reg_cos_sim}")
    assert delphi_log_reg_cos_sim > 9e-1, (
        f"trunc log reg cosine similarity is {delphi_log_reg_cos_sim}"
    )
...
def test_truncated_one_dimension_logistic_regression():
    ...
    trunc_sklearn_ = ch.from_numpy(
        np.concatenate([trunc_sklearn.coef_.flatten(), trunc_sklearn.intercept_])
    )
    print(f"trunc sklearn: {trunc_sklearn_}")
    trunc_sklearn_cos_sim = float(
        cos_sim(trunc_sklearn_[None, ...], sklearn_[None, ...])
    ...
    ch.manual_seed(seed)
    trunc_log_reg = TruncatedLogisticRegression(args, phi, alpha)
    trunc_log_reg.fit(x_trunc, y_trunc)
...
def test_truncated_one_dimension_probit_regression_no_intercept():
    ...
    pred = probit_results.predict(X) > 0.5
    acc = np.equal(pred, y.flatten()).sum() / len(y)
    ...
    pred = trunc_probit_results.predict(X) > 0.5
    ...
    ch.manual_seed(seed)
    trunc_prob_reg = TruncatedProbitRegression(args, phi, alpha, fit_intercept=False)
    ...
    delphi_prob_reg_cos_sim = float(
        cos_sim(delphi_prob_reg_[None, ...], probit_[None, ...])
    )
    print(f"delphi cos sim: {delphi_prob_reg_cos_sim}")
    assert delphi_prob_reg_cos_sim > 9e-1, (
        f"trunc prob reg cosine similarity is {delphi_prob_reg_cos_sim}"
    )
...
def test_truncated_one_dimension_probit_regression():
    ...
    probit_ = ch.from_numpy(probit_results.params)
    print(f"probit: {probit_}")
    pred = probit_results.predict(X_ones) > 0.5
    acc = np.equal(pred, y.flatten()).sum() / len(y)
    print(f"probit acc: {acc}")
    probit_conf_matrix = confusion_matrix(y, pred)
    print(f"probit confusion matrix: \n {probit_conf_matrix}")
    ...
    print(f"trunc probit: {trunc_probit_}")
    trunc_probit_cos_sim = float(cos_sim(trunc_probit_[None, ...], probit_[None, ...]))
    pred = trunc_probit_results.predict(X_ones) > 0.5
    trunc_probit_acc = np.equal(pred, y.flatten()).sum() / len(y)
    print(f"trunc probit acc: {trunc_probit_acc}")
    print(f"trunc probit cos sim: {trunc_probit_cos_sim}")
    trunc_probit_conf_matrix = confusion_matrix(y, pred)
    ...
    ch.manual_seed(seed)
    trunc_prob_reg = TruncatedProbitRegression(args, phi, alpha)
    trunc_prob_reg.fit(x_trunc, y_trunc)

    delphi_prob_reg_ = ch.cat(
        [trunc_prob_reg.coef_.flatten(), trunc_prob_reg.intercept_]
    )
    print(f"delphi prob reg: {delphi_prob_reg_}")
    delphi_prob_reg_pred = trunc_prob_reg.predict(X)
    delphi_prob_reg_acc = delphi_prob_reg_pred.eq(y).sum() / len(y)
...
def test_untruncated_multinomial_logistic_regression():
    ...
    ch.manual_seed(seed)
    delphi_log_reg = TruncatedLogisticRegression(
        args, phi, alpha, fit_intercept=False, multi_class="multinomial"
    )
    ...
    assert delphi_soft_cos_sim > 9e-1, (
        f"trunc multinomial log reg cosine similarity is {delphi_soft_cos_sim}"
    )
...
def test_truncated_multinomial_logistic_regression():
    ...
    ch.manual_seed(seed)
    delphi_softmax = SoftmaxRegression(args, fit_intercept=False)
    ...
    assert delphi_cos_sim > 9e-1, (
        f"trunc multinomial log reg cosine similarity is {delphi_cos_sim}"
    )
...
def test_truncated_multinomial_logistic_regression_labels():
    ...
    trunc_indices = (
        phi_logistic(sklearn_noised, y[..., None]).flatten().nonzero().flatten()
    )
    X_trunc, Y_trunc = X[trunc_indices], y[trunc_indices][..., None]
    alpha = X_trunc.size(0) / X.size(0)
    print(f"alpha: {alpha}")
