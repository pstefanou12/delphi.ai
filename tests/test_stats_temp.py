    ...
    def __call__(self, batch):
        ...
        if z.size(1) >= 5:
            prec1, prec5 = accuracy(
                ...
                targ.reshape(targ.size(0), 1).float(),
                topk=(1, 5),
            )
        else:
            prec1, prec5 = accuracy(
...
class TestStats(unittest.TestCase):
    ...
    def test_truncated_logistic_regression(self):
        ...
        result_store = Store(OUT_DIR + "some_momentum")
        result_store.add_table(
            "models",
            {
                "sklearn": "__object__",
                "sklearn_acc": float,
                "sklearn_conf_matrix": "__object__",
                "trunc_sklearn": "__object__",
                "trunc_sklearn_acc": float,
                "trunc_sklearn_conf_matrix": "__object__",
                "trunc_sklearn_cos_sim": float,
                "softmax_diff": "__object__",
                "softmax_acc": float,
                "softmax_conf_matrix": "__object__",
                "softmax_cos_sim": float,
                "softmax": "__object__",
                "gumbel_diff": "__object__",
                "gumbel_acc": float,
                "gumbel_conf_matrix": "__object__",
                "gumbel_cos_sim": float,
                "gumbel": "__object__",
                "trunc_log_reg": "__object__",
                "trunc_log_reg_acc": float,
                "trunc_log_reg_conf_matrix": "__object__",
                "trunc_log_reg_cos_sim": float,
                "trunc_multi_log_reg_diff": "__object__",
                "trunc_multi_log_reg_acc": float,
                "trunc_multi_log_reg_conf_matrix": "__object__",
                "trunc_multi_log_reg_cos_sim": float,
                "trunc_multi_log_reg": "__object__",
                "id_trunc_log_reg": "__object__",
                "id_trunc_log_reg_acc": float,
                "id_trunc_log_reg_conf_matrix": "__object__",
                "id_trunc_log_reg_cos_sim": float,
                "id_trunc_multi_log_reg_diff": "__object__",
                "id_trunc_multi_log_reg_acc": float,
                "id_trunc_multi_log_reg_conf_matrix": "__object__",
                "id_trunc_multi_log_reg_cos_sim": float,
        ...
        for i in range(trials):
            ...
            for c in C:
                ...
                sklearn_ = ch.from_numpy(
                    np.concatenate([sklearn.coef_.flatten(), sklearn.intercept_])
                )
                ...
                trunc_sklearn_ = ch.from_numpy(
                    np.concatenate(
                        [trunc_sklearn.coef_.flatten(), trunc_sklearn.intercept_]
                    )
                )
                print(f"trunc sklearn: {trunc_sklearn_}")
                trunc_sklearn_cos_sim = float(
                    cos_sim(trunc_sklearn_[None, ...], sklearn_[None, ...])
                )
                ...
                trunc_log_reg_ = ch.cat(
                    [trunc_log_reg.coef_.flatten(), trunc_log_reg.intercept_]
                )
                print(f"id trunc log reg: {trunc_log_reg_}")
                trunc_log_reg_cos_sim = float(
                    cos_sim(trunc_log_reg_[None, ...], sklearn_[None, ...])
                )
                ...
                print(
                    f"id trunc log reg confusion matrix: \n {trunc_sklearn_conf_matrix}"
                )
                result_store["models"].update_row(
                    {
                ...
                trunc_multi_log_reg_diff = ch.cat(
                    [
                        trunc_multi_log_reg.coef_[1] - trunc_multi_log_reg.coef_[0],
                        (
                            trunc_multi_log_reg.intercept_[1]
                            - trunc_multi_log_reg.intercept_[0]
                        )[..., None],
                    ]
                )
                trunc_multi_log_reg_ = ch.cat(
                    [
                        trunc_multi_log_reg.coef_,
                        trunc_multi_log_reg.intercept_[..., None],
                    ],
                    axis=1,
                )
                trunc_multi_cos_sim = float(
                    cos_sim(trunc_multi_log_reg_diff[None, ...], sklearn_[None, ...])
                )
                trunc_multi_log_reg_pred = trunc_multi_log_reg.predict(X_)
                trunc_multi_log_reg_acc = trunc_multi_log_reg_pred.eq(
                    y.flatten()
                ).sum() / len(y)
                ...
                trunc_multi_log_reg_conf_matrix = confusion_matrix(
                    y, trunc_multi_log_reg_pred
                )
                print(
                    f"id trunc multi log reg confusion matrix: \n {trunc_multi_log_reg_conf_matrix}"
                )
                result_store["models"].update_row(
                    {
                ...
                trunc_log_reg = stats.TruncatedLogisticRegression(
                    train_kwargs, weight=trunc_log_reg_[None, ...]
                )
                trunc_log_reg.fit(x_trunc, y_trunc)
                trunc_log_reg_ = ch.cat(
                    [trunc_log_reg.coef_.flatten(), trunc_log_reg.intercept_]
                ...
                ch.manual_seed(seed)
                trunc_multi_log_reg = stats.TruncatedLogisticRegression(
                    train_kwargs, weight=trunc_multi_log_reg_.T
                )
                trunc_multi_log_reg.fit(x_trunc, y_trunc.flatten().long())
                trunc_multi_log_reg_diff = ch.cat(
                    [
                ...
                print(
                    f"trunc multi log reg confusion matrix: \n {trunc_multi_log_reg_conf_matrix}"
                )
                result_store["models"].update_row(
                    {
                ...
                ch.manual_seed(seed)
                gumbel_model = GumbelCEModel(
                    train_kwargs, X.size(1) + 1, len(y.unique())
                )
                ...
                train_loader, val_loader = make_train_and_val(
                    train_kwargs, x_trunc_ones, y_trunc.flatten().long()
                )
                trainer.train_model((train_loader, val_loader))
                gumbel_diff = gumbel_model.model[1] - gumbel_model.model[0]
                gumbel_ = gumbel_model.model
                gumbel_cos_sim = float(
                    cos_sim(gumbel_diff[None, ...], sklearn_[None, ...])
                )
                ...
                softmax_cos_sim = float(
                    cos_sim(softmax_diff[None, ...], sklearn_[None, ...])
                )
