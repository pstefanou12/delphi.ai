...
* ``normalize`` (bool): our methods assume that the :math:`max(||x_{i}||_{2}) <= 1`, so before running the procedure, you must  divide the input featurers $\mathcal{X} = \{x_{(1)}, x_{(2)}, ... , x_{(n)}\}$ by $max(||x_{i}||_{2}) \dot \sqrt(k)$, where $\mathcal{k}$
