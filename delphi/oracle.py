...
triangle = lambda x: x[1]>=0 and x[1] <= +x[0] + 1 and x[1] <= 1- x[0] and not((x[0]/2)**2 + (x[1]-0.52)**2 <= 0.02)
...
class Unknown_Gaussian(oracle): 
    ...
    def __call__(self, x): 
        ...
        return x[((ch.exp(self.emp_dist.log_prob(x))/ch.exp(self.dist.log_prob(x)))*self.psi_k(x) > .5).flatten().nonzero(as_tuple=False).flatten()]
