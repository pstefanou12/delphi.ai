...
class TruncatedExponentialFamilyDistributionNLL(  # pylint: disable=abstract-method
    ...
    """
    Truncated negative log likelihood for multivariate normal with known truncation.

    Calculates the truncated NLL in forward and gradients w.r.t. mu and cov in backward.
    Samples batch_size * num_samples samples, filters those in the truncation set,
    and retains up to batch_size filtered samples. If fewer than batch_size remain,
    uses zeros and computes the untruncated log likelihood.
    """
...
class UnknownTruncationMultivariateNormalNLL(  # pylint: disable=abstract-method
    ...
    def forward(ctx, theta, data, phi, exp_h, dims):  # pylint: disable=invalid-name,arguments-differ,too-many-arguments,too-many-positional-arguments
        """
        Args:
            theta: current reparameterized mean and covariance matrix estimates concatenated
            data: precomputed gradient values for both the mean and covariance matrix
...
class SwitchGrad(ch.autograd.Function):  # pylint: disable=abstract-method
    ...
    def forward(  # pylint: disable=too-many-arguments,too-many-positional-arguments,too-many-locals,arguments-differ
        ...
        Args:
            ...
            num_samples: number of samples per batch element in rejection sampling
        ...
        # test whether to use censor-aware or censor-oblivious gradient
        M = ch.distributions.MultivariateNormal(  # pylint: disable=invalid-name
        ...
        # result and result_inv are masks to keep noised and unnoised samples
