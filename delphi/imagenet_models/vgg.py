"""VGG model implementations for ImageNet-scale datasets.
...
class VGG(nn.Module):
    """VGG network architecture for ImageNet-scale datasets."""

    def __init__(self, features, num_classes=1000, init_weights=True):
        """Initialize VGG with feature layers, avgpool, and classifier head."""
        super().__init__()
    ...
    def forward(self, x, with_latent=False, fake_relu=False, no_relu=False):  # pylint: disable=arguments-differ
        """Forward pass through VGG with optional latent output."""
    ...
    def _initialize_weights(self):
        """Initialize model weights using Kaiming normal for conv layers."""
...
def make_layers(cfg, batch_norm=False):
    """Build a Sequential of conv/pool layers from a config list."""
    layers = []
    in_channels = 3
    for v in cfg:
        if v == "M":
...
def _vgg(arch, cfg, batch_norm, pretrained, progress, **kwargs):
    """Internal helper to construct a VGG model, optionally loading pretrained weights."""
    if pretrained:
...
def vgg19(pretrained=False, progress=True, **kwargs):
    r"""VGG 19-layer model (configuration "E") from
    `"Very Deep Convolutional Networks For Large-Scale Image Recognition"
    <https://arxiv.org/pdf/1409.1556.pdf>`_
